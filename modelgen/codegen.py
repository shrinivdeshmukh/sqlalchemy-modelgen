...
from modelgen import unicode_literals, division, print_function, absolute_import
...
{models}"""
    ...
    def render_metadata_declarations(self, outfile: str=None):
        if 'sqlalchemy.ext.declarative' in self.collector:
            return 'Base = declarative_base()\nmetadata = Base.metadata'
        if outfile.endswith('.py'):
            return f"{outfile.split('.py')[0].split('/')[-1]}_metadata = MetaData()"
    ...
    def render_table(self, model, outfile=None):
        if outfile.endswith('.py'):
            metadata = f"{outfile.split('.py')[0].split('/')[-1]}_metadata"
        else:
            metadata = 'metadata'
        rendered = 't_{0} = Table(\n{2}{1!r}, {3},\n'.format(
            model.name, model.table.name, self.indentation, metadata)
    ...
    def render(self, outfile=sys.stdout, outfile_name=None):
        rendered_models = []
        for model in self.models:
            if isinstance(model, self.class_model):
                ...
                rendered_models.append(self.render_table(model, outfile=outfile_name))

        output = self.template.format(
            imports=self.render_imports(),
            metadata_declarations=self.render_metadata_declarations(outfile=outfile_name),
